...
- **AstrBot**（插件宿主；本插件以 AstrBot 插件形式注册，`metadata.yaml` 声明 `version: 1.3.0`）。
- **Python 3.10+** —— 源码使用现代类型标注与 `asyncio`；本机在 **Python 3.12** 上实测通过（三套自测 146/93/15 全绿）。
- **运行依赖：无第三方包**（只用标准库 + AstrBot 提供的 API）。
- **需要联网**：查询与推送都要访问社区/官方数据源（米游社、TapTap 等）；本仓另有若干 `probe_*.mjs` 开发期脚本，非运行必需。

**English:**
- **AstrBot** (the plugin host; this plugin registers as an AstrBot plugin and declares `version: 1.3.0` in `metadata.yaml`).
- **Python 3.10+** — the source uses modern type annotations and `asyncio`; verified on **Python 3.12** here (its three self-test suites pass 146/93/15).
- **Runtime dependencies: none** (standard library plus the APIs AstrBot provides).
- **Networking required**: both querying and pushing reach community/official data sources (miyoushe, TapTap, …); the `probe_*.mjs` files in this repo are development-time scripts, not needed at runtime.
